...
!     TEMP CHP Add printout for DEMAND variables
      ...
      CHARACTER*10 OUTDM  !DEMAND.OUT
      INTEGER NOUTDG, ERRNUM, YRPLT, YEAR, DOY, YRDOY, DAP, TIMDIF
      REAL PGAVL_SAVE
      LOGICAL FEXIST
...
!       Initialize daily root growth output file      
        INQUIRE (FILE = OUTDM, EXIST = FEXIST)
        IF (FEXIST) THEN
          OPEN (UNIT = NOUTDG, FILE = OUTDM, STATUS = 'OLD',
     &      IOSTAT = ERRNUM, POSITION = 'APPEND')
        ELSE
          OPEN (UNIT = NOUTDG, FILE = OUTDM, STATUS = 'NEW',
     &      IOSTAT = ERRNUM)
          WRITE(NOUTDG,'("*DEMAND OUTPUT FILE")')
        ENDIF
  ...
  200   FORMAT('@YEAR DOY   DAS   DAP'
     &  ,'      NDMTOT      NDMOLD',
     &   '      NDMVEG      AGRVG2        FRLF       FRSTM        FRRT',
     &   '      CDMVEG        XFRT       PGAVL      CDMREP',
     &   '      NDMREP       GDMSD',
     &   '       GDMSH      ADDSHL       CDMSH      AGRSH2')
...
!     TEMP CHP
      IF (YRDOY > 2005184) THEN
        WRITE(5678,'(A10, 2I8, 3F12.6)') 'DEMAND 0  ',
     &    YRDOY, DAS, NR2
      ENDIF
!     END TEMP CHP
!======================================================
...
!     TEMP CHP
      IF (YRDOY > 2005184) THEN
        WRITE(5678,'(A10, 3I8, 10F12.6)') 'DEMAND 1  ',
     &    YRDOY, NPP, NAGE, PAGE, LNGSH, SHELN(NPP), GRRAT1, LNGPEG
      ENDIF
!     END TEMP CHP
!======================================================
...
!     TEMP CHP
      IF (YRDOY > 2005184) THEN
        WRITE(5678,'(A10, 2I8, 10F12.6)') 'DEMAND 2  ',
     &    YRDOY, NPP, ADDSHL, GRRAT1, SHELN(NPP)
      ENDIF
!     END TEMP CHP
!======================================================
...
!     TEMP CHP
      IF (YRDOY > 2005184) THEN
        WRITE(5678,'(A10, 2I8, 10F12.6)') 'DEMAND 3  ',
     &    YRDOY, NPP, ADDSHL, GRRAT1, SHELN(NPP), SHLAG
      ENDIF
!     END TEMP CHP
!======================================================
...
!     TEMP CHP
!     Save the value of PGAVL at the time of this calculation, not at the
!     time of prinout. They will be different.
      PGAVL_SAVE = PGAVL
!     END TEMP CHP
