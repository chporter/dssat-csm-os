...
!     TEMP CHP Add printout for roots variables
      ...
      CHARACTER*9 OUTVR
      INTEGER NOUTDG, ERRNUM, YRPLT, YEAR, DOY, DAP, TIMDIF
      LOGICAL FEXIST
...
!=========================================================================
!     TEMP CHP Add printout for veggr variables

          OUTVR  = 'VEGGR.OUT'
          CALL GETLUN('OUTVR',  NOUTDG)
...
!       Initialize daily root growth output file      
        INQUIRE (FILE = OUTVR, EXIST = FEXIST)
        IF (FEXIST) THEN
          OPEN (UNIT = NOUTDG, FILE = OUTVR, STATUS = 'OLD',
     &      IOSTAT = ERRNUM, POSITION = 'APPEND')
        ELSE
          OPEN (UNIT = NOUTDG, FILE = OUTVR, STATUS = 'NEW',
     &      IOSTAT = ERRNUM)
          WRITE(NOUTDG,'("*VEGGR OUTPUT FILE")')
        ENDIF
...
!     TEMP CHP
!     This removes some of the release vs debug issue, but not all
!     This solves the RDPD issue!
        IF (NAVL < 1.E-6) THEN
          NAVL = 0.0
        ENDIF
!     END TEMP CHP
